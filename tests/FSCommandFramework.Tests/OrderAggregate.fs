...
module OrderAggregate =
    ...
    let dispatch (state: OrderState option) (command: OrderCommand) : Result<OrderEvent list, string> =
        ...
        | CancelOrder(_, reason) ->
            ...
            | Some s when s.Status = "cancelled" -> Error $"Order '{s.Id}' has already been cancelled."
            | Some s when s.Status = "shipped" -> Error $"Order '{s.Id}' has already been shipped and cannot be cancelled."
            | Some s -> Ok [ OrderCancelled(s.Id, reason) ]

        | ShipOrder(_, trackingNumber) ->
            match state with
            | None -> Error "Order does not exist."
            | Some s when s.Status = "cancelled" -> Error $"Order '{s.Id}' has been cancelled and cannot be shipped."
    ...
    let apply (state: OrderState option) (event: OrderEvent) : OrderState =
        ...
        | OrderCancelled _ -> { state.Value with Status = "cancelled" }
        | OrderShipped _ -> { state.Value with Status = "shipped" }

    let deserializeCommand = ReflectionDeserializer.forCommands<OrderCommand> None
    let deserializeEvent = ReflectionDeserializer.forEvents<OrderEvent> None
              ...
              task {
                  ...
                  | OrderCancelled(orderId, _) ->
                      do! conn.ExecuteAsync(
                              ...
                              {| orderId = orderId; now = DateTimeOffset.UtcNow |},
                              tx
                          ) |> Ignore
